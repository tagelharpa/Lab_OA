...
int main()
{
	...
	fill_vector(vec);
	print_vector(vec, "Начальный вектор: ");

	add_max_to_begin(vec);
	print_vector(vec, "Максимальный объект добавлен в начало: ");

	erase_min(vec);
	print_vector(vec, "Удаляется минимальный объект: ");

	inc_average(vec);
	print_vector(vec, "Среднее арифметическое для объектов: ");
